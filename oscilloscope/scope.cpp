...
QRect Scope::drawForeground(void) {
    ...
    for(std::list<Channel>::iterator i = channels.begin(), iend = channels.end();i != iend;++i) {
        ...
        painter.setPen(i->getPen());
        x = width();
        y = round(height()/2-scale*(i->data[(data_idx - 1 + i->data.size())%i->data.size()]+i->offset));
        if(y < miny) miny = y;
        if(y > maxy) maxy = y;
        painter.moveTo(x,y);
        for(size_t j = 1;j<i->data.size();++j) {
            x = width() - round(((j*period)*width())/(hScl*divX));
            y = round(height()/2-scale*(i->data[(data_idx-j-1+i->data.size())%i->data.size()]+i->offset));
            if(y < miny) miny = y;
            if(y > maxy) maxy = y;
            painter.lineTo(x,y);
            if(x < 0) break;
